...
import { LF, SP as SP$1, TB, QT, DASH, RT, CO as CO$1, COSP } from '@spare/enum-chars';
import * as ColumnsMapper from '@vect/columns-mapper';
import { mapper as mapper$6 } from '@vect/columns-mapper';
...
fluoVector.bind(MUTATE_PIGMENT$2);
...
class XrStream extends Callable {
  ...
  br(...items) {
    return this.queue.push(items.map(parenth$2).join(CO$1)), this;
  }
...
function strategies({
  ...
  const eta = new Eta(),
        ...
        prettyNames = functionNames.map(ros).join(CO$1),
  ...
  if (showAverage) lapse.unshiftRow('average', fluoVector(mapper$6(tmx, average)));
...
const average = nums => round$1(nums.reduce((a, b) => a + b, 0) / nums.length);
